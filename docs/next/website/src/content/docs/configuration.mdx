...
Mouse selections stay highlighted after copying. With `ui.copy_on_select = true` (the default), releasing a selection copies its text. Set it to `false` to copy only on request. Ctrl+C copies a visible selection without clearing it. Cmd+C does the same when the terminal forwards it to Herdr. Without a selection, Ctrl+C reaches the pane.

Ordinary typing clears the selection. Direct and prefix plugin bindings capture its text before clearing it. Selections still clear when pane, geometry, or content changes invalidate them.
...
The sidebar is the main Herdr dashboard. In the expanded desktop sidebar, the new-space and menu buttons sit directly below the sidebar heading. Search `ui.` in the [Config reference](/docs/config-reference/) for sizing, collapsed mode, Agent panel ordering, mouse behavior, pane borders, and other presentation settings.
...
To put tabs below Spaces, open Settings with `ctrl+b`, release, then `s`. Select vertical tabs, choose enabled, and apply. You can also configure:
...
The default is `false`. The expanded desktop sidebar shows a separate Tabs section for the active Space, with the new-tab button directly below its heading. Drag its heading to resize the section. Wheel input and its scrollbar scroll the list without changing the active tab. Clicking, context menus, drag reordering, and keyboard tab navigation remain available. The Agents section stays below Tabs when enabled. Long tab titles wrap at `·`, spaces, and hyphens, with continuation lines aligned under the title. Words wider than the section split between complete characters. Each tab is limited to the section's visible height.

Collapsed, hidden, or too-short sidebars use horizontal tabs. Mobile keeps its switcher. `hide_tab_bar_when_single_tab` also applies to vertical tabs. The top or bottom strip remains available for status and mode indicators, so switching orientation does not resize terminal panes. Saved names and Auto Title are unchanged.
...
Sidebar rows and desktop tabs show agent status with compact colored dots by default. Tab markers use the combined status of their panes and do not change saved tab names. To distinguish blocked, working, done, idle, and unknown states by shape as well as color, choose distinct symbols in Settings or configure:
...
To hide the global Agents list in the desktop sidebar, open Settings with `ctrl+b`, release, then `s`. Select the agents panel section, choose hidden, and apply. The choice is saved immediately without restarting the server or stopping agents.
...
The default is `true`. Hiding the panel gives its height to Spaces, shared with Tabs when vertical tabs are enabled. Agent processes, space status indicators, notifications, and the mobile switcher remain unchanged. Config file edits take effect when the client reloads config.
