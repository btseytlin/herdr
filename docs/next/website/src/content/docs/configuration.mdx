...
The default is `false`. The expanded desktop sidebar shows a separate Tabs section for the active Space. Drag its heading to resize the section. Wheel input and its scrollbar scroll the list without changing the active tab. Clicking, context menus, drag reordering, and keyboard tab navigation remain available. The Agents section stays below Tabs when enabled. Long tab titles wrap at `·`, spaces, and hyphens, with continuation lines aligned under the title. Words wider than the section split between complete characters. Each tab is limited to the section's visible height.
